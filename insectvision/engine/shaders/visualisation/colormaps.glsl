...
vec3 apply_colormap(float t, int colormap_id) {
    if (colormap_id == 0)      return colormap_diverging(t);
    else if (colormap_id == 1) return colormap_sequential(t);
    else                       return colormap_thermal(t);
}

/// Normalise raw scalar value to [0, 1] (with dynamic-range compression)
float normalize_scalar(float raw, float data_min, float data_max, float compression, int colormap_id) {
