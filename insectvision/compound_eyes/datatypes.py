...
RCPT_STATIC_DTYPE = np.dtype([
    ...
    ('sensitivity', np.float32),    # 4 bytes: sensitivity multiplier
...
RCPT_DYNAMIC_DTYPE = np.dtype([
    ...
    ('pad1', np.float32),           # 4 bytes of padding
...
# TODO: Expand RCPT_STATIC_DTYPE to 64 bytes to include:
#   ('spectral_sensitivity', np.float32, 3) -> multipliers for UV, Blue, Green
#   ('adaptation_state', np.float32)        -> current neural/biochemical adaptation
